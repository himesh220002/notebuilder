        ...
        <div className="min-h-screen bg-[#f1f5f9] font-sans text-slate-900">
            ...
            <main className="max-w-[1300px] mx-auto px-6 py-16">
                ...
                <header className="mb-16 md:mb-24 text-center">
                    <div className="inline-flex items-center gap-2 px-4 sm:px-6 py-2 bg-emerald-100 text-emerald-700 rounded-full text-[10px] sm:text-xs font-black uppercase tracking-widest mb-6 md:mb-8 border border-emerald-200 shadow-sm">
                    ...
                    <h1 className="text-4xl sm:text-6xl md:text-7xl lg:text-8xl font-black text-slate-900 mb-6 md:mb-8 tracking-tighter">
                        Taxation & <br />
                        <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-600 via-indigo-600 to-sky-600 italic text-5xl sm:text-7xl md:text-8xl lg:text-9xl">Financial Plan</span>
                    </h1>
                ...
                <section className="mb-20 md:mb-28">
                    <div className="bg-white rounded-[2rem] sm:rounded-[3rem] shadow-2xl border border-slate-100 p-6 sm:p-12 relative overflow-hidden">
                        <div className="absolute top-0 right-0 p-4 sm:p-8 opacity-5">
                            <Truck className="w-32 h-32 sm:w-64 sm:h-64 text-emerald-900" />
                        </div>

                        <div className="flex items-center gap-4 sm:gap-6 mb-12 sm:mb-16 relative">
                            <div className="w-14 h-14 sm:w-20 sm:h-20 bg-emerald-900 text-white rounded-2xl sm:rounded-3xl flex items-center justify-center text-xl sm:text-3xl font-black shadow-2xl rotate-3">1</div>
                            <div>
                                <h2 className="text-2xl sm:text-4xl font-black text-slate-900 italic leading-tight">GST Essentials</h2>
                                <p className="text-slate-400 font-bold uppercase text-[9px] sm:text-[10px] tracking-[0.2em]">Goods and Service Tax</p>
                            </div>
                        </div>
                        ...
                        <div className="grid lg:grid-cols-1 gap-16">
                            <div className="space-y-8">
                                ...
                                <div className="p-6 sm:p-8 bg-indigo-900 rounded-2xl sm:rounded-3xl text-white relative overflow-hidden">
                                    <div className="absolute top-0 right-0 p-4 sm:p-8 opacity-10">
                                        <ArrowRightLeft className="w-16 h-16 sm:w-32 sm:h-32" />
                                    </div>
                                    <h3 className="text-xl sm:text-2xl font-black mb-6 italic underline decoration-indigo-400 decoration-4 underline-offset-8">Input Tax Credit (ITC)</h3>
                                    <p className="text-indigo-200 text-sm sm:text-base mb-8 max-w-2xl font-medium">
                                    ...
                                    <div className="flex items-center gap-4 m-1 p-3 bg-indigo-300 rounded-2xl w-fit mb-8 max-w-full overflow-x-auto">
                                        <FormulaBlock
                ...
                <section className="mb-20 md:mb-28">
                    <div className="flex items-center gap-4 mb-12">
                        ...
                        <h2 className="text-2xl sm:text-4xl font-black text-slate-900 tracking-tight italic leading-tight">Income Tax Framework</h2>
                    </div>
                    ...
                    <div className="grid lg:grid-cols-2 gap-12 mt-20">
                        <div className="bg-white rounded-[2rem] sm:rounded-[3rem] p-6 sm:p-12 border border-slate-100 shadow-xl relative group">
                            <div className="absolute top-0 right-0 p-6 sm:p-10 opacity-5">
                                <Gem className="w-32 h-32 sm:w-48 sm:h-48 text-indigo-900" />
                            </div>
                            <h3 className="text-2xl sm:text-3xl font-black mb-6 sm:mb-10 flex items-center gap-3">
                                <ShieldCheck className="w-6 h-6 sm:w-8 sm:h-8 text-indigo-600" />
                                Smart Deductions
                            </h3>
                            <div className="grid gap-4 sm:gap-6">
                                {taxationData.incomeTax.deductions.map((d, i) => (
                                    <div key={i} className="flex gap-4 sm:gap-6 p-4 sm:p-6 bg-slate-50 rounded-xl sm:rounded-2xl border border-transparent hover:border-indigo-100 hover:bg-white transition-all">
                                        <div className="w-16 sm:w-20 text-indigo-600 font-black text-[10px] sm:text-xs uppercase pt-1">{d.section}</div>
                                        <div>
                                            <div className="text-[9px] sm:text-[10px] font-black text-slate-400 uppercase mb-1">Limit: {d.limit}</div>
                                            <p className="text-xs sm:text-sm text-slate-700 font-bold italic">{d.items}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                        ...
                        <div className="space-y-8">
                            <div className="bg-slate-900 rounded-[2rem] sm:rounded-[3rem] p-6 sm:p-12 text-white shadow-2xl relative overflow-hidden h-full flex flex-col justify-center">
                                <div className="absolute bottom-0 right-0 p-6 sm:p-10 opacity-10">
                                    <HandCoins className="w-48 h-48 sm:w-64 sm:h-64 text-emerald-400" />
                                </div>
                                <h3 className="text-2xl sm:text-3xl font-black mb-6 sm:mb-8 italic text-emerald-400 flex items-center gap-2 sm:gap-3">
                                    <Calculator className="w-5 h-5 sm:w-6 sm:h-6" /> Method & Example
                                </h3>
                                <div className="space-y-4 sm:space-y-6 relative mb-8 sm:mb-12">
                                    <div className="flex gap-3 sm:gap-4">
                                        <div className="w-6 h-6 sm:w-8 sm:h-8 bg-emerald-500 rounded-full flex items-center justify-center font-black text-[10px] sm:text-xs shrink-0">1</div>
                                        <p className="font-bold text-sm sm:text-base">Gross Income minus Standard Deduction (₹50,000).</p>
                                    </div>
                                    <div className="flex gap-3 sm:gap-4">
                                        <div className="w-6 h-6 sm:w-8 sm:h-8 bg-emerald-500 rounded-full flex items-center justify-center font-black text-[10px] sm:text-xs shrink-0">2</div>
                                        <p className="font-bold text-sm sm:text-base">Subtract legal deductions (80C, 80D, etc.).</p>
                                    </div>
                                    <div className="flex gap-3 sm:gap-4">
                                        <div className="w-6 h-6 sm:w-8 sm:h-8 bg-amber-500 rounded-full flex items-center justify-center font-black text-[10px] sm:text-xs shrink-0">3</div>
                                        <p className="font-bold text-sm sm:text-base">Apply Slab Rates + 4% Cess on tax.</p>
                                    </div>
                                </div>

                                <div className="bg-white/5 p-4 sm:p-8 rounded-[1.5rem] sm:rounded-[2rem] border border-white/10 space-y-4">
                                    <span className="text-[9px] sm:text-[10px] font-black text-emerald-400 uppercase tracking-widest block mb-2">Solved Illustration</span>
                                    <div className="text-xs sm:text-sm space-y-4">
                                        ...
                                        <div className="space-y-2 sm:space-y-3 font-bold">
                                            ...
                                            <div className="bg-black/30 p-3 sm:p-4 rounded-xl space-y-1 sm:space-y-2 mt-4 text-[10px] sm:text-xs">
                                                ...
                                                <div className="flex justify-between text-emerald-400 font-black text-xs sm:text-sm pt-2"><span>Total Tax:</span> <span>₹33,800</span></div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                ...
                <section className="mb-28">
                    <div className="flex flex-col md:flex-row items-end justify-between gap-6 mb-16">
                        <div>
                            <span className="text-[10px] sm:text-xs font-black text-emerald-600 uppercase tracking-[0.3em] block mb-2">Finance Laboratory</span>
                            <h2 className="text-4xl sm:text-6xl font-black text-slate-900 tracking-tighter italic leading-tight">Tax <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-500 to-indigo-500">Calculus</span></h2>
                        </div>
                    ...
                    <div className="mt-24 space-y-12">
                        <header className="text-center">
                            ...
                            <p className="text-slate-400 font-bold uppercase text-[9px] sm:text-[10px] tracking-widest underline decoration-emerald-200 underline-offset-4">Board & Previous Year Questions</p>
                        </header>
                        <div className="grid md:grid-cols-2 gap-8">
                            {examProblems.map((prob, idx) => (
                                <div key={idx} className="bg-white p-6 sm:p-10 rounded-[2rem] sm:rounded-[2.5rem] shadow-xl border border-slate-50 hover:border-emerald-200 transition-all hover:shadow-2xl relative group">
                                    <div className="absolute top-0 right-0 p-4 sm:p-6">
                                        <span className="px-3 py-1 bg-emerald-100 text-emerald-700 rounded-lg text-[9px] sm:text-[10px] font-black uppercase tracking-tighter ring-1 ring-emerald-200">
                                            {prob.examTag}
                                        </span>
                                    </div>
                                    <h4 className="text-lg sm:text-xl font-bold text-slate-900 mb-6 sm:mb-8 pr-16 sm:pr-20 leading-snug">{prob.question}</h4>
                                    <div className="bg-slate-50 p-4 sm:p-6 rounded-xl sm:rounded-2xl border-l-4 border-indigo-600">
                                        <span className="text-indigo-600 font-black text-[9px] sm:text-[10px] uppercase tracking-widest block mb-3 sm:mb-4">Master Solution</span>
